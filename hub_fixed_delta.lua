...
task.spawn(function()
    ...
    refreshBtn.MouseButton1Click:Connect(function()
        ...
        for _, player in ipairs(Players:GetPlayers()) do
            if Config.HideKawaifuFromPanel and isKawaifuUser(player) then  end
...
task.spawn(function()
    ...
    while true do
        task.wait(1)
        if not Config.AutoResetOnBalloon then  end
...
if owner.UserId == LocalPlayer.UserId then return end
        ...
        for slot,ad in pairs(al) do
            if type(ad)=="table" then
                local nm=ad.Index
                local info=AnimalsData and AnimalsData[nm]; if not info then  end
...
RunService.Heartbeat:Connect(function(dt)
    ...
    -- en AUTO mode. On garde le pct actuel et on  depuis là.
    if target~=stealTarget then
        ...
        if not target then
            ...
            -- Si "charging" :  de charger normalement
        end
    end
